...
####윤재
# <<<------------현주_1-------------->>>
# @blueprint.route('/juthor_category/<string:f_id>')
# def category(f_id):
#     db = pymysql.connect(**config)
#     cur = db.cursor()
#     ##숫자숫자

#     sql = '''select category ,count(category)from store where festival_id=%s group by category'''
#     cur.execute(sql, [f_id])
#     data_list = cur.fetchall()
#     return render_template('juthor_category.html', data_list=data_list)


@blueprint.route('/juthor_storeList/<string:f_id>')
@requires_auth
def store_list(f_id):
    db = pymysql.connect(**config)
    cur = db.cursor()
    ...
    data_list = cur.fetchall()
    return render_template('juthor_storeList.html',
                           segment='storelist',
                           data_list=data_list)
...
def menulist(id):
    ...
    return render_template('juthor_storemenulist.html',
                           segment='storemenulist',
                           data_list=data_list)
